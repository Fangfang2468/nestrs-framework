//! `#[bind]` 的 `Provider::Bound` 生成。
//!
//! bind 的语义不是注册一份 trait-object 实例。它只将具体服务的 Arena 地址通过
//! Rust 类型系统投影为 `dyn Trait`，从而保存正确的 vtable。请求 key 在未来的
//! provider 选择阶段按 `InheritRequestedKey` 继承到 concrete 服务 token。

use zyn::{syn, zyn};

/// 输出一个 trait 到 concrete provider 的 typed bound 注册。
///
...
mod tests {
    ...
    fn emits_a_bound_provider_with_typed_projectors_and_inherited_key_policy() {
        ...
        assert!(rendered.contains("REFLECTED_PROVIDERS"));
        assert!(rendered.contains("Provider :: Bound"));
